...
export function shouldTagElement(elementName: string): boolean {
        ...
        walk(ast, {
          enter(node: any) {
            ...
            if (node.type === 'JSXOpeningElement') {
              ...
              const attributes = jsxNode.attributes.reduce((acc: Record<string, string>, attr: any) => {
  ...
  try {
    ...
    try {
      const userConfig = await import(
        `${tailwindIntermediateFile}?update=${Date.now()}` // cache buster
      );
